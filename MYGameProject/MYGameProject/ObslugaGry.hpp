...
public:
    ...
    void wyswietlOdliczanie(sf::Sprite& TloGry, Gracz1& player1, Gracz2& player2, Pilka& ball, SpeedUp& Item_Up, SpeedDown& Item_Down) {
        sf::Text odliczanieText;
        sf::Font font;
        font.loadFromFile(fontPath.string());
        odliczanieText.setFont(font);
        odliczanieText.setCharacterSize(100);
        odliczanieText.setFillColor(sf::Color::Black);
        odliczanieText.setPosition(350, 250);

        for (int i = 3; i > 0; --i) {
            odliczanieText.setString(std::to_string(i));
    ...
    void graj() {
        ...
        // Wyœwietl odliczanie przed rozpoczêciem gry
        wyswietlOdliczanie(TloGry, player1, player2, ball, Item_Up, Item_Down);
